...
class VerticalShapeTransform {
 public:
   Pixel ComputeGradientIntersect(uint16_t curr_point, uint16_t max_points,
                                  const Pixel &start_color,
   ...
   Pixel ComputePixelColor(const LinearRange &range, uint16_t curr_pixel,
                           uint16_t pixels_to_draw) {
      switch (range.type) {
      case NORMAL:
         return range.start_color;
      case GRADIENT:
         return ComputeGradientIntersect(curr_pixel, pixels_to_draw,
                                         range.start_color, range.end_color);
      }
   }
   ...
   void DrawRange(const LinearRange &range, Pixel *hw_pixels,
      ...
      for (uint16_t p = start_pixel; p <= end_pixel; p++) {
         // Do not render pixels out of range unless we are wrapping.
         // In the case of wrapping, draw at the beginning.
         if (range.IsWrapped() || p < hw_pixel_count) {
            hw_pixels[p % hw_pixel_count] =
                ComputePixelColor(range, curr_pixel, pixels_to_draw);
         }
         curr_pixel++;
      }
   }
